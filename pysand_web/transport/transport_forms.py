...
class TransportHydroForm(TransportBaseForm):
    ...
    rho_l = DecimalField('Liquid Density [kg/m³]', id='pvt', default=700, places=0, validators=[DataRequired(), NumberRange(min=1, max=1500)])
...
class TransportStokesForm(TransportBaseForm):
    rho_m = DecimalField('Mixture Density [kg/m³]', id='pvt', default=200, places=0, validators=[DataRequired(), NumberRange(min=1, max=1500)])
    mu_m = DecimalField('Mixture Viscosity [cP]', id='pvt', default=0.01, places=3, validators=[DataRequired(), NumberRange(min=0.001, max=10)])
    angle = DecimalField('Inclination from vertical (\u03B1) [deg]', id='specific', places=0, default=50, validators=[InputRequired(), NumberRange(min=0, max=80)])